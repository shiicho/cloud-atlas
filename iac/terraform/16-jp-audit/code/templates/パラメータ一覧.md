...
## 7. State Locking パラメータ

> **Note**: Terraform 1.10+ では S3 原生锁定（`use_lockfile = true`）を使用します。

| パラメータ名 | 値 | 備考 |
|-------------|---|------|
| use_lockfile | true | S3 原生 锁定 |
| lock file | .tflock | S3 上に作成 |

**旧方式（参考）**: Terraform 1.9 以下では別の方式を使用していました
