...
> **前置**：已完成 [01](../01-cfn-deploy/)、[02](../02-session-manager/)；目标实例为"受管实例（Managed instance）"。  
...
参考 **[02 · 使用 Session Manager 免密登录 EC2（浏览器 Shell）](../02-session-manager/)** 进入会话管理页面。
在 Shell 中输入：
```bash
...
```
![填写安装命令](./img/05_commands_install.png)

**操作**：**目标（Targets）** → **手动选择目标** → 勾选实验机

![选择目标并运行](./img/06_targets_run.png)

**操作**：在"输出选项" **取消勾选** 「启用 S3 存储桶写入」，其他保持默认，点击页面右下角 **运行（Run）**。
**备注**：本课仅看控制台输出；S3/CloudWatch 日志将在未来课程中统一配置。

![选择目标并运行](./img/07_output_options.png)

**期望**：输出包含 transaction summary，结尾 **Complete!**

![安装输出结果](./img/08_output_install_1.png)
![安装输出结果](./img/08_output_install_2.png)
![安装输出结果](./img/08_output_install_3.png)
...
![浏览器 Shell：执行后验证](./img/09_postcheck_shell.png)
...
* **Package not found**：Ubuntu 请改用：
  ...
  ```
* **无输出**：命令仍在执行；刷新命令详情；或检查是否把输出发往 CloudWatch/S3（见 [05 课](../05-session-logging/)）。

## 下一步
- **[04 · Parameter Store（创建/读取/在脚本中使用）](../04-parameter-store/)**
- **[05 · 会话/命令日志落地](../05-session-logging/)**
