...
| `log.count[file,regexp]` | 统计匹配行数 | 返回数值，减少 DB 负担 |
| `logrt.count[regexp,regexp]` | 统计轮转日志匹配数 | 适合告警计数场景 |

> 💡 **推荐**：如果只需要统计 ERROR 数量触发告警，优先使用 `log.count[]` 而非 `log[]`，可大幅减少数据库存储。
...
**1. Zabbix 的限制**：Zabbix 截至 7.0 版本**没有原生 journald 支持**。
社区功能请求（ZBXNEXT-7907）自 2022 年开放至今未解决。

**2. rsyslog 作为桥梁**：这是生产环境的标准模式，不是临时方案。
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  systemd-journald          rsyslog              Zabbix      │
│  ┌─────────────┐      ┌─────────────┐      ┌─────────────┐ │
│  │ 二进制日志   │      │ imjournal   │      │ log[] Item  │ │
│  │ /var/log/   │ ──── │ 模块读取    │ ──── │ 监控文本    │ │
│  │ journal/    │      │ 写入文本    │      │ 日志文件    │ │
│  └─────────────┘      └─────────────┘      └─────────────┘ │
│        ↓                    ↓                    ↓         │
│   现代 Linux 默认      /var/log/messages      触发告警      │
│                        /var/log/secure                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**3. 日本企业环境**：`/var/log/secure` 监控是 ISMS（ISO 27001）合规要求。
认证日志审计是日本 IT 运维的基本职责（運用監視の基本）。

> 📖 **深入了解**：参见 [lab-log-context.md](./lab-log-context.md) 了解行业实践背景

**在 Monitored Host 上执行**：
...
**设置 Zabbix Agent 读取权限**：

```bash
# 设置日志文件权限（默认 600，改为 644）
...
> ⚠️ **maxlines 含义**：`maxlines` 是**每秒**最多处理的行数，而非每次采集。高频日志场景需适当提高此值。

3. 点击「Add」

### 2.2 logrt[] 轮转日志示例

对于按日期轮转的日志（如 `app.log.2024-01-01`），使用 `logrt[]`：
...
> 💡 **logrt[] 文件名模式**：第一个参数是正则表达式，`\.` 转义点号，`.*` 匹配日期后缀。
...
| 注意点 | 说明 |
|--------|------|
| **Timeout** | 确保命令在 Timeout 内完成（Zabbix 7.0 支持 Item 级 Timeout） |
| **返回值** | 必须返回正确类型（数字/字符串） |
| **重启** | 修改后需重启 Agent（或用 `zabbix_agent2 -R userparameter_reload`） |
| **安全** | 避免执行用户输入，使用白名单（见下方详细说明） |
| **测试** | 先用 `zabbix_agent2 -t` 测试 |

### 3.6 UserParameter 安全警告

> ⚠️ **命令注入风险**：带参数的 UserParameter 可能被恶意利用

**危险示例**（❌ 不要这样做）：
```ini
# 危险！直接使用用户输入作为命令参数
UserParameter=custom.exec[*],/usr/bin/myapp $1
# 攻击者可能传入: ; rm -rf / 或 $(cat /etc/passwd)
```

**安全做法**（✅ 推荐）：
```ini
# 方法 1：使用 DenyKey 限制危险 Key（推荐）
DenyKey=system.run[*]
...
# safe_wrapper.sh 内部进行输入验证
```

**AllowKey/DenyKey 配置**（Zabbix 7.0 推荐）：
```ini
...
```bash
# 确认 snmpd 已安装（CloudFormation 应已安装）
...
> 💡 **生产环境安全建议**：SNMPv2c 使用明文 community string，生产环境应使用 SNMPv3（支持加密和认证）。
