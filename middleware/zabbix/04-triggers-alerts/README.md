# 04 · 触发器与告警通知（Triggers & Alerts）

> **目标**：配置触发器、邮件通知和维护窗口
> **前置**：[03 · 基础监控 + 死活检查](../03-monitoring-basics/)
> **费用**：实验环境持续产生费用（约 $0.03/小时）；完成系列后请删除堆栈
> **时间**：30-35 分钟
> **实战项目**：配置磁盘告警 + Golden Week 维护窗口

## 将学到的内容

1. 创建自定义触发器
2. 理解 Severity 级别和依赖关系
3. 配置 Email 通知
4. 创建 Action（触发器 → 通知）
5. 设置维护窗口（Maintenance Window）
6. 实践告警确认（Acknowledgment）

---

## Step 1 — 创建自定义触发器

### 1.1 触发器工作原理

```
┌─────────────────────────────────────────────────────────────┐
│                    Trigger 工作流程                          │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Item 采集数据                                               │
│       │                                                      │
│       ▼                                                      │
│  ┌─────────────────┐     ┌─────────────────┐                │
│  │ Expression 评估 │────►│ 状态: OK/PROBLEM │               │
│  │ last() < 20     │     └─────────────────┘                │
│  └─────────────────┘              │                          │
│                                   ▼                          │
│                         ┌─────────────────┐                  │
│                         │ Event 生成      │                  │
│                         └─────────────────┘                  │
│                                   │                          │
│                                   ▼                          │
│                         ┌─────────────────┐                  │
│                         │ Action 执行     │                  │
│                         │ (发送通知)      │                  │
│                         └─────────────────┘                  │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### 1.2 创建磁盘空间触发器

> 💡 **模板已包含触发器**：
> `Linux by Zabbix agent active` 模板已内置磁盘空间触发器（`FS [/]: Space is critically low`、`FS [/]: Space is low`）。
> 本节创建自定义触发器是为了**学习触发器配置流程**。生产环境中，优先使用或修改模板内置触发器。

1. 「Data collection」→「Hosts」→ 点击 `monitored-host-01`
2. 切换到「Triggers」标签页
3. 点击「Create trigger」

**基本配置**：

| 字段 | 值 |
|------|-----|
| Name | `Disk space is low on {HOST.NAME} ({ITEM.LASTVALUE})` |
| Severity | `Warning` |
| Expression | 见下方 |

**构建 Expression**：

点击「Add」打开表达式构建器：

| 字段 | 值 |
|------|-----|
| Item | `FS [/]: Space: Used, in %` |
| Function | `last()` |
| Result | `>=80` |

完整表达式示例：
```
last(/monitored-host-01/vfs.fs.dependent.size[/,pused])>=80
```

> 💡 **Item Key 说明**：
> `Linux by Zabbix agent active` 模板使用 Dependent Item Discovery 模式，磁盘空间百分比的 Key 是 `vfs.fs.dependent.size[/,pused]`（注意 `dependent` 前缀），而不是旧版的 `vfs.fs.size[/,pused]`。

**Recovery expression**（防止 Flapping）：
```
last(/monitored-host-01/vfs.fs.dependent.size[/,pused])<75
```

> 💡 **Hysteresis（滞后）**：Problem 阈值 80%，Recovery 阈值 75%，避免在临界值附近反复触发

4. 点击「Add」

### 1.3 创建 High 级别触发器（用于邮件测试）

> 💡 **为什么需要两个触发器？**
> - Warning (80%): 早期预警，运维人员关注
> - High (90%): 紧急告警，触发邮件通知
>
> 后续 Step 4 会配置通知规则（Action），设为 `Severity >= High`，只有 High 级别以上才会发送邮件。

重复上述步骤创建第二个触发器：

| 字段 | 值 |
|------|-----|
| Name | `Disk space is critically low on {HOST.NAME} ({ITEM.LASTVALUE})` |
| Severity | `High` |
| Expression | `last(/monitored-host-01/vfs.fs.dependent.size[/,pused])>=90` |
| Recovery expression | `last(/monitored-host-01/vfs.fs.dependent.size[/,pused])<85` |

点击「Add」完成创建。

> ✅ **验证**：「Data collection」→「Hosts」→ 点击 Host →「Triggers」标签页，确认两个触发器已创建。

**触发器总结**：

| 触发器 | 阈值 | Severity | 邮件通知 |
|--------|------|----------|----------|
| Disk space is low | >= 80% | Warning | ❌ |
| Disk space is critically low | >= 90% | High | ✅ |

### 1.4 Severity 级别

| 级别 | 颜色 | 用途 |
|------|------|------|
| Not classified | 灰色 | 未分类 |
| Information | 浅蓝 | 信息提示 |
| Warning | 黄色 | 警告，需关注 |
| Average | 橙色 | 一般严重 |
| High | 红色 | 严重问题 |
| Disaster | 深红 | 灾难级别 |

> 🇯🇵 **日本 IT 現場での対応レベル**：
>
> | Severity | 対応区分 | SLA目安 | 通知方法 |
> |----------|---------|---------|----------|
> | Disaster | 重大障害（P1） | 即時対応 | 電話連絡必須 |
> | High | 緊急障害（P2） | 15分以内 | SMS + チャット |
> | Average | 通常障害（P3） | 1時間以内 | メール + チケット |
> | Warning | 軽微（P4） | 翌営業日 | メールのみ |

---

## Step 2 — 理解触发器依赖

### 2.1 依赖关系场景

```
场景：网络设备故障导致所有服务器不可达

不使用依赖：
  - Switch down ← 触发
  - Server A unreachable ← 触发
  - Server B unreachable ← 触发
  - Server C unreachable ← 触发
  = 大量重复告警！

使用依赖：
  - Switch down ← 触发
  - Server A unreachable ← 被抑制（依赖 Switch）
  - Server B unreachable ← 被抑制
  - Server C unreachable ← 被抑制
  = 只收到 1 个告警
```

### 2.2 配置依赖

1. 编辑 Trigger
2. 切换到「Dependencies」标签页
3. 点击「Add」选择父触发器
4. 保存

> 💡 **Lab 环境限制**：本实验环境为双主机结构，难以演示真实的多层依赖场景。
>
> **生产环境典型用例**：
>
> | 模式 | 父触发器 | 子触发器 | 效果 |
> |------|----------|----------|------|
> | 网络层级 | Core Switch down | Server unreachable | 交换机故障时，抑制服务器告警 |
> | 阈值级联 | 磁盘 Critical (90%) | 磁盘 Warning (80%) | 避免同时触发两个告警 |
> | 服务依赖 | Database down | Application down | DB 故障时，抑制应用告警 |
> | 维护窗口 | Maintenance active | Service alerts | 计划维护期间抑制告警 |
>
> **日本运维现场**称此为「障害連鎖」（故障连锁）对策，是防止告警风暴的必备配置。
>
> 💡 **可测试场景**：Step 5 将配置维护窗口，届时可观察告警抑制效果。

---

## Step 3 — 配置 Email 通知

> 💡 **本课使用 Lab 环境**：实验环境预装了 Mailpit（假 SMTP 服务器），无需配置真实邮箱即可测试邮件通知。
> 生产环境配置请参考 [3.5 节](#35-生产环境参考可选)。

### 3.1 了解 Mailpit（Lab 环境）

Mailpit 是一个轻量级假 SMTP 服务器：
- 捕获所有发送的邮件（不会发送到外部）
- 提供 Web UI 查看邮件内容
- 适合开发和测试环境

**访问 Mailpit Web UI**：

```bash
# 查看 Mailpit Web UI 地址
aws cloudformation describe-stacks --stack-name zabbix-lab \
  --query 'Stacks[0].Outputs[?OutputKey==`MailpitWebURL`].OutputValue' \
  --output text
```

示例：`http://<ZabbixServerIP>:8025`

在浏览器中打开此地址，稍后所有 Zabbix 发送的邮件都会显示在这里。

### 3.2 配置 Zabbix SMTP

1. 「Alerts」→「Media types」
2. 点击「Email」（系统预置）

配置 SMTP 指向 Mailpit：

| 字段 | 值 |
|------|-----|
| SMTP server | `localhost` |
| SMTP server port | `1025` |
| SMTP helo | `zabbix-lab.local` |
| SMTP email | `zabbix@zabbix-lab.local` |
| Connection security | `None` |
| Authentication | `None` |

3. 点击「Update」保存

### 3.3 测试邮件发送

1. 返回 Media types 列表
2. 在 Email 行的「Action」列（最右侧，可能需要横向滚动）点击「Test」
3. 输入任意邮箱地址（如 `test@example.com`），点击「Test」
4. 在 Mailpit Web UI 中确认收到测试邮件

> 💡 **Test 按钮位置**：Test 功能在 Media types **列表页**的 Action 列，不在编辑对话框内。
> 这是 Zabbix 4.2+ 以来的标准设计。

### 3.4 为用户配置 Media

配置哪些用户接收邮件通知：

1. 「Users」→「Users」
2. 点击你的用户名（如 `Admin` 或 `ops-test`）
3. 切换到「Media」标签页
4. 点击「Add」

| 字段 | 值 |
|------|-----|
| Type | Email |
| Send to | `admin@zabbix-lab.local`（任意地址，Mailpit 会捕获） |
| When active | 1-7,00:00-24:00（全天候） |
| Use if severity | 勾选 Warning 及以上级别 |

5. 点击「Add」→「Update」

### 3.5 生产环境参考（可选）

> ⚠️ **本节仅供参考**，Lab 环境请使用上述 Mailpit 配置。

**Gmail SMTP**：

| 字段 | 值 |
|------|-----|
| SMTP server | smtp.gmail.com |
| SMTP server port | 587 |
| SMTP helo | gmail.com |
| SMTP email | your-email@gmail.com |
| Connection security | STARTTLS |
| Authentication | Username and password |
| Username | your-email@gmail.com |
| Password | App password（非登录密码） |

> 💡 **Gmail 注意**：需要启用 2FA 并创建 App Password。进入 Google 账户 → 安全 → 应用专用密码

**AWS SES**：

| 字段 | 值 |
|------|-----|
| SMTP server | email-smtp.ap-northeast-1.amazonaws.com |
| SMTP server port | 587 |
| Connection security | STARTTLS |
| Username | SES SMTP username |
| Password | SES SMTP password |

> 💡 **SES 注意**：新账户在 Sandbox 模式，只能发送到已验证邮箱。需申请生产环境访问。

---

## Step 4 — 创建 Action

Action 定义「触发器触发时做什么」。

### 4.1 创建通知 Action

1. 「Alerts」→「Actions」→「Trigger actions」
2. 点击「Create action」

**Action 标签页**：

| 字段 | 值 |
|------|-----|
| Name | `Notify on High severity problems` |
| Conditions | 添加条件（见下） |

**添加 Conditions**：
- Trigger severity `>=` `High`
- Host group `=` `Lab/Linux servers`（使用 Lesson 02 创建的 Host Group）

**Operations 标签页**：

点击「Add」添加操作：

| 字段 | 值 |
|------|-----|
| Operation type | Send message |
| Send to user groups | 选择用户组（如 Zabbix administrators） |
| Send only to | Email |

**Default subject**：
```
[{TRIGGER.SEVERITY}] {TRIGGER.NAME}
```

**Default message**：
```
Host: {HOST.NAME}
Problem: {TRIGGER.NAME}
Severity: {TRIGGER.SEVERITY}
Time: {EVENT.DATE} {EVENT.TIME}
Current value: {ITEM.LASTVALUE}

Original problem ID: {EVENT.ID}
```

**Recovery operations 标签页**：

添加恢复通知：
- Operation type: Send message
- 使用相同用户组

**Recovery subject**：
```
[RESOLVED] {TRIGGER.NAME}
```

**Recovery message**：
```
Host: {HOST.NAME}
Problem: {TRIGGER.NAME}
Status: RESOLVED

Resolved at: {EVENT.RECOVERY.DATE} {EVENT.RECOVERY.TIME}
Duration: {EVENT.DURATION}

Original problem ID: {EVENT.ID}
```

3. 点击「Add」

### 4.2 测试告警

#### 4.2.1 理解磁盘使用率

首先在 Monitored Host 上查看当前磁盘状态：

```bash
df -h /
```

输出示例：
```
Filesystem      Size  Used Avail Use% Mounted on
/dev/nvme0n1p1  8.0G  1.8G  6.3G  22% /
```

**各列含义**：

| 列 | 含义 | 示例 |
|----|------|------|
| Filesystem | 设备名 | /dev/nvme0n1p1 |
| Size | 总容量 | 8.0G |
| Used | 已使用 | 1.8G |
| Avail | 可用空间 | 6.3G |
| Use% | 使用率 | 22% |
| Mounted on | 挂载点 | / |

**计算示例**（手动方式）：
- 总容量 8GB，当前使用 22%（1.8GB）
- 目标 85%：需要 8GB × 85% = 6.8GB
- 需要额外创建：6.8GB - 1.8GB = **5GB 文件**

#### 4.2.2 使用辅助脚本

为方便测试，我们提供了辅助脚本自动计算并创建测试文件。

> ⚠️ **在 Monitored Host 上执行**（不是 Zabbix Server）

```bash
# 下载脚本（如果尚未克隆仓库）
cd ~
curl -O https://raw.githubusercontent.com/shiicho/cloud-atlas/main/middleware/zabbix/04-triggers-alerts/code/fill-disk.sh
chmod +x fill-disk.sh

# 查看当前状态和帮助
./fill-disk.sh
```

> 💡 **脚本特点**：
> - 自动计算所需文件大小
> - 文件保存在用户目录（无需 sudo）
> - 支持小数格式（85 或 0.85）
> - 显示详细的计算过程

#### 4.2.3 测试 Warning 触发器（80%）

```bash
# 填充到 85%
./fill-disk.sh 85

# 确认使用率
df -h /
```

等待 1-2 分钟，在 Zabbix Web UI 检查：

1. 「Monitoring」→「Problems」
2. 确认看到 **Warning** 级别告警：`Disk space is low on monitored-host-01`
3. 检查 Mailpit：**不会收到邮件**（Action 过滤了 Warning 级别）

#### 4.2.4 测试 High 触发器（90%）+ 邮件通知

```bash
# 填充到 95%
./fill-disk.sh 95

# 确认使用率
df -h /
```

等待 1-2 分钟，在 Zabbix Web UI 检查：

1. 「Monitoring」→「Problems」
2. 确认看到 **High** 级别告警：`Disk space is critically low on monitored-host-01`
3. 检查 Mailpit Web UI：**应收到邮件通知** ✅

> 💡 **为什么只有 High 收到邮件？**
> Step 4.1 创建的 Action 条件是 `Severity >= High`。
> - Warning (80%): 显示在 Problems，但不发邮件
> - High (90%): 显示在 Problems，且发送邮件

#### 4.2.5 清理测试文件

```bash
# 删除测试文件
./fill-disk.sh clean

# 确认恢复
df -h /
```

等待 1-2 分钟，确认：
1. 「Monitoring」→「Problems」中告警自动恢复（Recovery）
2. Mailpit 收到恢复通知邮件

---

## Step 5 — 配置维护窗口

> 场景：Golden Week（日本黄金周）期间抑制告警

### 5.1 创建维护窗口

1. 「Data collection」→「Maintenance」
2. 点击「Create maintenance period」

**Maintenance 标签页**：

| 字段 | 值 |
|------|-----|
| Name | `Lab Test Maintenance` |
| Maintenance type | `With data collection`（继续采集但不告警） |
| Active since | （当前时间） |
| Active till | （当前时间 + 10 分钟） |

> 💡 **Lab 测试**：设置 10 分钟窗口便于立即验证效果。生产环境可设置更长时间（如 Golden Week 5 天）。

> ⚠️ **时区注意**：维护窗口时间基于 Zabbix Server 时区。确保已在 [01 · Server 初始化](../01-server-setup/) 中设置为 Asia/Tokyo。

**Periods 标签页**：

点击「Add」：
- Period type: One time only
- Date: （今天日期）
- Maintenance period length: 10 minutes

**Hosts & groups 标签页**：

- Host groups: 选择需要维护的组
- 或 Hosts: 选择特定主机

3. 点击「Add」

### 5.2 维护窗口类型

| 类型 | 数据采集 | 告警 | 用途 |
|------|----------|------|------|
| With data collection | ✅ | ❌ | 计划内停机、假期 |
| No data collection | ❌ | ❌ | 硬件维护、网络切割 |

### 5.3 验证维护窗口

1. 在维护期间，主机图标显示维护标记
2. 「Problems」中告警被抑制（但仍记录）
3. 维护结束后自动恢复

---

## Step 6 — 告警确认（Acknowledgment）

> 💡 **前提条件**：需要有活跃的 Problem 才能练习确认操作。
> 如果之前已清理完毕，可用 `./fill-disk.sh 85` 重新触发一个告警。

### 6.1 确认问题

1. 「Monitoring」→「Problems」
2. 在问题行的「Actions」列点击「Update」
3. 在「Update problem」对话框中：
   - 勾选「Acknowledge」复选框
   - 在「Message」中描述处理情况
   - 可选操作：
     - Change severity（调整严重级别）
     - Suppress（暂时抑制告警）
     - Close problem（手动关闭）
4. 点击「Update」提交

> 💡 **Zabbix 7.0 UI**：告警确认通过「Update」对话框完成，
> 提供更多选项（调整严重级别、抑制、关闭）在同一界面中。

### 6.2 确认流程最佳实践

```
┌─────────────────────────────────────────────────────────────┐
│                  告警处理流程                                │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  1. 收到告警 ──► 2. 确认收到（Ack）                         │
│                      │                                       │
│                      ▼                                       │
│               3. 开始处理（记录）                            │
│                      │                                       │
│                      ▼                                       │
│               4. 问题解决                                    │
│                      │                                       │
│               ┌──────┴──────┐                               │
│               │             │                                │
│               ▼             ▼                                │
│          自动恢复      手动关闭                              │
│         (Recovery)   (Close problem)                         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

---

## Mini-Project：完整告警链路配置

> 场景：为 Lab 环境配置完整的磁盘告警链路

### 要求

1. **创建触发器**
   - Warning: 磁盘 > 80%
   - High: 磁盘 > 90%
   - Disaster: 磁盘 > 95%

2. **配置 Action**
   - 为每个严重级别创建独立 Action
   - 使用 Mailpit 验证邮件发送

3. **配置维护窗口**
   - 设置「从现在起 5 分钟后」的短窗口
   - 抑制非 Disaster 级别告警
   - 观察窗口期间的行为差异

4. **测试**
   - 使用 `fill-disk.sh` 触发各级别告警
   - 在 Mailpit Web UI 验证通知
   - 验证维护窗口抑制效果

> 💡 **进阶通知渠道**：生产环境中常用 Slack、PagerDuty、Opsgenie 等即时通知。
> Zabbix 7.0 内置 40+ Webhook 模板，配置方法参考官方文档。

---

## 面试问答

### Q: トリガーのフラッピングを防ぐには？

**A**:
- **ヒステリシス設定**: Problem expression と Recovery expression で異なる閾値を設定（例：80% で警告、75% で回復）
- **{$THRESHOLD} マクロ**: 閾値をマクロ化して調整しやすく
- **Multiple PROBLEM events**: 同じトリガーからの重複イベント生成を制御
- **nodata() 関数**: データ欠損時の誤検知を防止

### Q: Maintenance Window の使い方は？

**A**:
- **計画停止**: サーバーメンテナンス、パッチ適用時
- **定期メンテ**: 毎週日曜深夜など定期作業時間
- **長期休暇**: Golden Week、年末年始など
- **With/Without data collection**: データ継続の必要性で選択
- **一時抑制**: 誤報対応中の一時的な抑制にも使用

---

## 常见错误

| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 邮件未发送 | SMTP 配置错误 | 使用 Test 功能验证 |
| Action 不触发 | Conditions 不匹配 | 检查严重级别、Host group 条件 |
| 维护窗口无效 | 时区设置错误 | 确认 Zabbix 时区为 Asia/Tokyo |
| 重复告警 | 未配置 Recovery | 添加 Recovery expression |

### 排查 Action

1. 「Reports」→「Action log」
2. 查看 Action 执行历史
3. 检查失败原因

---

## 本课小结

| 概念 | 要点 |
|------|------|
| Trigger | 基于 Item 数据的告警条件 |
| Expression | 触发条件表达式 |
| Recovery | 恢复条件，防止 Flapping |
| Severity | 6 个级别，用于分级响应 |
| Action | 触发器 → 通知的映射 |
| Maintenance | 计划内抑制告警 |
| Acknowledgment | 告警确认和处理记录 |

---

## 清理提醒

> ⚠️ **费用提醒**：实验环境持续产生费用。完成整个系列后，请删除 CloudFormation 堆栈。
> 详见 → [00 · 清理资源](../00-architecture-lab/#清理资源)

---

## 下一步

告警已配置！下一课我们将学习日志监控、自定义指标和 SNMP 入门。

→ [05 · 日志 + 自定义指标](../05-logs-custom/)

## 系列导航

← [03 · 基础监控](../03-monitoring-basics/) | [系列首页](../) | [05 · 日志 + 自定义指标](../05-logs-custom/) →
