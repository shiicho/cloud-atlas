...
# 3. 进入本课目录
cd ~/02a-ssm-connection

# 4. 确认 Lesson 02 的 Managed Nodes 正在运行
ansible all -m ping
```
...
**解决方案**：大文件直接使用 `amazon.aws.s3_object` 模块，不走 SSM 通道。
...
| **コスト考慮** | S3 传输有成本，大文件考虑直接使用 `amazon.aws.s3_object` 模块 |
...
> ⚠️ **SSM 专用 S3 Bucket 会持续产生费用**

完成本课后，如果不再需要 SSM 连接功能，请清理以下资源：

```bash
# 1. 删除 S3 Bucket（需要先清空）
BUCKET_NAME="my-ansible-ssm-bucket-$(aws sts get-caller-identity --query Account --output text)"
aws s3 rm s3://${BUCKET_NAME} --recursive
aws s3 rb s3://${BUCKET_NAME}

# 2. 如果部署了专用 EC2 实例，也请删除
# aws cloudformation delete-stack --stack-name ansible-lesson-02a
```

> 💡 **保留主线课程资源**：Lesson 02 创建的 Managed Nodes（web-1, db-1）请保留，后续课程继续使用。
