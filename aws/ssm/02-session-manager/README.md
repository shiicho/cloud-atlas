...
![01 — 会话管理器首页](./img/01.png)

## Step 2 — 选择目标实例
**操作**：在 **托管实例** 列表中，勾选你的实验机（标签包含 `atlas-ec2`，或用 `InstanceId` 搜索）。
**参考**：**[01 · CloudFormation 部署最小实验环境](../01-cfn-deploy/) 中创部署的EC2实例，CloudFormation输出中有 InstanceId**

![02 — 选择托管实例](./img/02_select_instance.png)

## Step 3 — 开始会话
**操作**：点击右上角 **开始会话**。

![03 — 点击"开始会话"](./img/03_click_start_session.png)
...
![04 — 浏览器 Shell 界面](./img/04_browser_shell.png)

## 下一步
- **[03 · Run Command 批量执行脚本（安装 htop）](../03-run-command/)**
- **[04 · Parameter Store 参数存储](../04-parameter-store/)**
